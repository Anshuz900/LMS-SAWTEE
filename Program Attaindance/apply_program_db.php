...
if ($new > $maximum_leaves) {
    ...
    if ($leave_duration == "one_day") {
        ...
        if (mysqli_num_rows($result) == 0) {
            $sql3 = "INSERT INTO lms.leave_requests 
                     (staff_id, leave_type, start_date, end_date, days_requested, date_applied, leave_status, program_attendance)
                     VALUES 
                     ('$staff_id', '$leave_type', '$leave_date', '$leave_date', '1', '".date("Y-m-d")."', '$status', '$program_attendance')";
    ...
    } elseif ($leave_duration == "multiple_days") {
        ...
        if (mysqli_num_rows($result1) == 0 && mysqli_num_rows($result2) == 0) {
            $sql3 = "INSERT INTO lms.leave_requests 
                     (staff_id, leave_type, start_date, end_date, days_requested, date_applied, leave_status, program_attendance)
                     VALUES 
                     ('$staff_id', '$leave_type', '$start_date', '$end_date', '$no_of_days', '".date("Y-m-d")."', '$status', '$program_attendance')";
            mysqli_query($connection, $sql3) or die(mysqli_error($connection));
            echo "<script>
                    alert('Leave Request Submitted.');
                    window.location=\"apply_leave.php\";
